...
class BookingDAO(BaseDAO):
    ...
    async def delete_booking(cls,booking_id:int, user_id:int):
        async with async_session_maker() as session:
            exam = await session.execute(select(Bookings).where(and_(Bookings.id==booking_id,Bookings.user_id==user_id)))
            if len(list(exam))>0:
                query = delete(Bookings).where(and_(Bookings.id==booking_id,Bookings.user_id==user_id))
                await session.execute(query)
                await session.commit()

                return {
                    'status':status.HTTP_200_OK,
                    'detail': 'Бронь была успешно удалена'
                }
            else:
